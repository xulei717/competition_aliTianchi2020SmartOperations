...
class BasicModel:
    # lightgbm模型
    def lightgbm_model(self, train, test, features, target, n_splits, pred_threshold):
        print('lightgbm_model!!!')
        fold = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=2020)
        ...
        pred = [1 if p >= pred_threshold else 0 for p in pred]
        result['pred'] = pred
        print(result.shape)
