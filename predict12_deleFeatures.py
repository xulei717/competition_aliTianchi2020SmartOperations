...
# 第一类和第二类预测结果，总结果数：(17, 4) (378, 4) (380, 4)
...
for index, (train_idx, val_idx) in enumerate(fold.split(X, y)):
    ...
    joblib.dump(model, path + '/models/model1_dels_' + str(val_f1) + '.txt')
...
print(result.shape)  # (103, 5)
result[["manufacturer", "model", "serial_number", "dt"]].to_csv(path + "/result/result1_dels.csv", index=False, header=None)
result = result.groupby(["manufacturer", "model", "serial_number"])["dt"].min().reset_index()
result.to_csv(path + '/result/result1_dels_min.csv', index=False, header=None)
print(result.shape)  # (17, 4)
...
for index, (train_idx, val_idx) in enumerate(fold.split(X, y)):
    ...
    val_f1 = metrics.f1_score(val_y, val_pred)
    print(index, 'val f1', val_f1)
    joblib.dump(model, path + '/models/model2_dels_' + str(val_f1) + '.txt')
...
result = result[result.pred == 1]
print(result.shape)  # (4805, 5)
result[["manufacturer", "model", "serial_number", "dt"]].to_csv(path + "/result/result2_dels.csv", index=False, header=None)
result = result.groupby(["manufacturer", "model", "serial_number"])["dt"].min().reset_index()
result.to_csv(path + '/result/result2_dels_min.csv', index=False, header=None)
print(result.shape)  # (378, 4)
...
# 把一、二类预测结果进行合并
pre1 = pd.read_csv(path + '/result/result1_dels_min.csv', names=["manufacturer", "model", "serial_number", "dt"])
pre2 = pd.read_csv(path + '/result/result2_dels_min.csv', names=["manufacturer", "model", "serial_number", "dt"])
...
pre.to_csv(path + '/result/result12_dels_min.csv', index=False, header=None)
print(pre1.shape, pre2.shape, pre.shape)  # (17, 4) (378, 4) (380, 4)
...
print(td.shape)
rt12 = pd.read_csv(path + '/result/result12_dels_min.csv', names=["manufacturer", "model", "serial_number", "dt"])
...
tr.to_csv(path + '/result/result12_dels_max_test.csv', index=False, header=None)
print(tr.shape)
tr.head()
...
tr.drop_duplicates(inplace=True)
tr.reset_index(drop=True, inplace=True)
tr.to_csv(path + '/result/result12_dels_min_test.csv', index=False, header=None)
print(tr.shape)
tr.head()
